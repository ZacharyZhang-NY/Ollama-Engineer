...
def get_installed_models():
    try:
        ...
        # Process stdout to extract model names
        ...
        print(colored(f"Error listing models: {e}", "red"))
        logging.error(f"Error listing models: {e}")
        return []

def select_model(models):
    if not models:
        print(colored("No models available to select.", "red"))
        return None
...
def chat_with_ai(user_message, is_edit_request=False, retry_count=0, added_files=None):
    global last_ai_response, conversation_history, selected_model
    try:
        if added_files:
        ...
        if not is_edit_request:
            history = "\n".join(
                [f"User: {msg}" if i % 2 == 0 else f"AI: {msg}" for i, msg in enumerate(conversation_history)]
            )
            if history:
                user_message = f"{history}\nUser: {user_message}"

        if is_edit_request:
            prompt_text = EDIT_INSTRUCTION_PROMPT if retry_count == 0 else APPLY_EDITS_PROMPT
        ...
        if is_edit_request and retry_count == 0:
            ...
            print(colored("Ollama engineer is thinking...", "magenta"))
            logging.info("Sending general query to AI.")

        # Use subprocess to call Ollama with the correct command
        command = ['ollama', 'run', selected_model]

        result = subprocess.run(command, input=message_content, capture_output=True, text=True)
        if result.returncode != 0:
            print(colored(f"Error while communicating with Ollama: {result.stderr}", "red"))
            logging.error(f"Error while communicating with Ollama: {result.stderr}")
            return None
        else:
            last_ai_response = result.stdout.strip()
        ...
        print(colored(f"Error while communicating with Ollama: {e}", "red"))
...
def main():
    ...
    print(colored("Ollama engineer is ready to help you.", "cyan"))
    ...
    if not selected_model:
        print(colored("No model selected. Exiting.", "red"))
        return
...
"""
            ...
            if ai_response:
                print("Ollama engineer: Here are the suggested edit instructions:")
            ...
            if ai_response:
                while True:
                    print("Ollama engineer: Here is the suggested creation structure:")
                    rprint(Markdown(ai_response))
            ...
            if ai_response:
                print()
                print(colored("Ollama engineer: Here is your detailed plan:", "blue"))
                rprint(Markdown(ai_response))
            ...
            if ai_response:
                print()
                print(colored("Ollama engineer:", "blue"))
                rprint(Markdown(ai_response))
